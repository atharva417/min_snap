...
t=[0.2,0.4,0.8,1]   #notice there will be m+1 elements in time and position input arrays
x=[0,2,4,10]
y=[2,20,4,10]
z=[20,15,6,2]
Q_1=form_Q(1)   
...
print(type(Q),'typeofQ')
A=comp_A(t)
print(type(A),'typeofA')
...
for v in range(m):
    ...
    for i in range(100):
        ...
        for j in range(n*v,(v+1)*n):
            g=g+(p_x[j]*pow(r[i],j-(n*v)))
            e=e+(p_y[j]*pow(r[i],j-(n*v)))
            f=f+(p_z[j]*pow(r[i],j-(n*v)))
        w.append(g)
        u.append(e)
        a.append(f)
    ax.plot3D(w, u, a, 'r')
